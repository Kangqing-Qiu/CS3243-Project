...
public class PlayerSkeleton {
    ...
    public Boolean LegalMvChecking(final int orient,final int slot,int[][] PlayField,final int[] PlayTop,final State Nows) {
        //the final keyword is used in several contexts to define an entity that can only be assigned once.
        //state Nows ??
        int p
        Width[][] = Nows.getpWidth();
    ...
    public int pickMove(State s, int[][] legalMoves) extends AbstractList{
        //变量声明
        ...
        for(int MoveCount = 0; MoveCount < legalMoves.length; MoveCount ++) {
            ...
            if(LegalMvChecking(orient, slot, PlayField, PlayTop, s)){
                double TempScore = FindFitness(PlayField, PlayTop, TempWeights);

                if(Math.abs(score - highestScore) < 0){
                    possibleMoves.add(i);
                }
